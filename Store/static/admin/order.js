...
$.get(url_list, function (response, status) {
  ...
  JSON.parse(response).forEach(function (order) {
    ...
    row +=
      "<td style='display:none' class='date_delivery_o'>" +
      date_delivery +
      "</td>";
    row +=
      "<td style='display:none' class='products_id'>" +
      products_id.toString() +
      "</td>";

    row += "<td>";
    row +=
      "<button type='button' class='check btn btn-primary' data-bs-toggle='modal' data-bs-target='#checkmodal'>بررسی سفارش</button>";
    row += "</td>";
    row += "</tr>";
    $tbody.append(row);
  });
...
function checkRow() {
  $("tbody tr td:last-child button").click(function () {
    ...
    for (oid in products) {
      ...
      row += "<tr>";
      var str_href =
        "/product/?name=" + $name_product + "&price=" + $price_product;
      row +=
        "<td class='name_p'><a href='" +
        str_href +
        "'>" +
        $name_product +
        "</a></td>";
